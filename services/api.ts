...
import type { Admin, Campaign, Company, Researcher, SurveyResponse, Voucher, Question, QuestionOption, User } from '../types';
...
export const getResearcherRoute = async (researcherId: string): Promise<{ lat: number; lng: number; timestamp: number }[] | null> => {
...
export const updateResearcherRoute = async (researcherId: string, newPoint: { lat: number; lng: number; timestamp: number }) => {
    const { data, error } = await supabase
        .from('pesquisadores')
        .select('rota')
        .eq('id', researcherId)
    ...
    const currentRoute = data.rota || [];
    // Limita o histórico aos últimos 500 pontos para não sobrecarregar
    const newRoute = [...currentRoute, newPoint].slice(-500); 

    const { error: updateError } = await supabase
        .from('pesquisadores')
        .update({ rota: newRoute })
        .eq('id', researcherId);
