...
import { getResearchers, getFullCampaigns, getResponses, getResearcherRoute } from '../../services/api';
...
declare var L: any; // Declara a variável global L do Leaflet
...
const ResearcherTrackingModal: React.FC<{ researcher: Researcher | null; onClose: () => void; }> = ({ researcher, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (mapElement && !mapRef.current) {
            const map = L.map('tracking-map').setView([-15.7801, -47.9292], 4); // Centro do Brasil
        ...
        const fetchRoute = async () => {
            ...
            try {
                const data = await getResearcherRoute(researcher.id);
                if (data && data.length > 0) {
                    setRoute(data);
                    const lastPoint = data[data.length - 1];
                    setLastUpdated(new Date(lastPoint.timestamp).toLocaleString('pt-BR'));
                }
            } catch (error) {
        ...
        fetchRoute();
        const interval = setInterval(fetchRoute, 15000); // Atualiza a cada 15 segundos
    ...
    useEffect(() => {
        ...
        if (polylineRef.current) {
            ...
            polylineRef.current = L.polyline(latLngs, { color: '#3B82F6' }).addTo(mapRef.current);
        }

        // Atualiza o marcador da última posição
        if (markerRef.current) {
            markerRef.current.setLatLng(lastPoint);
        } else {
            markerRef.current = L.marker(lastPoint).addTo(mapRef.current);
        }

        markerRef.current.bindPopup(`<b>Última atualização:</b><br>${lastUpdated}`).openPopup();
    ...
    const isOpen = !!researcher;

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Rastreando ${researcher?.name}`}>
            <div className="w-full h-[60vh] relative">
                {isLoading && (
                    <div className="absolute inset-0 bg-light-background/80 dark:bg-dark-card/80 flex items-center justify-center z-10">
                        <LoadingSpinner text="Buscando localização" />
                    </div>
                )}
                <div id="tracking-map" className="w-full h-full rounded-md" style={{zIndex: 1}}></div>
                {lastUpdated && (
                     <div className="absolute top-2 right-2 bg-light-background dark:bg-dark-card p-2 rounded-md shadow-lg text-xs z-10">
...
const AdminResearchersPage: React.FC = () => {
    ...
    const [trackingResearcher, setTrackingResearcher] = useState<Researcher | null>(null);
    ...
    const handleOpenTrackingModal = (researcher: Researcher) => {
        setTrackingResearcher(researcher);
    };
    ...
    return (
        <div>
            ...
            <ResearcherTrackingModal researcher={trackingResearcher} onClose={() => setTrackingResearcher(null)} />
                    ...
                    return (
                        <div key={r.id} className="bg-light-background dark:bg-dark-card rounded-lg shadow-md flex flex-col">
                            ...
                            <footer className="px-5 pb-4 flex justify-end items-center gap-2">
                                <button onClick={() => handleOpenTrackingModal(r)} className="p-2 text-gray-500 hover:text-light-primary hover:bg-gray-200 dark:hover:bg-dark-border rounded-full transition-colors" title="Rastrear Pesquisador">
